...
const uint32_t MAX_SYNC_BYTE_READS = 20480;
...
FLACDecoderResult FLACDecoder::frame_sync_(){
    ...
    uint32_t count = 0;
    while( count++ < MAX_SYNC_BYTE_READS){
...
FLACDecoderResult FLACDecoder::decode_frame_header_(){
  ...
  //Assuming that we hav bits_per_sample from header
  // skip reading it here
  ...
  // todo: check for invalid codes, i.e. 0xffffffff and 0xffffffffffffffff
  
  //not used for now:
  //bool variable_block_size_mode = raw_header[1] & 0x01;
  
  uint32_t next_int = this->read_aligned_byte();
  raw_header[raw_header_len++] = next_int;
  uint8_t utf8_bytes_read = 1;
  while (next_int >= 0b11000000 && utf8_bytes_read < 7) {
  ...
  if(FLAC__crc8( raw_header, raw_header_len) != crc_read) {
		return FLAC_DECODER_ERROR_BAD_HEADER;;
	}
...
FLACDecoderResult FLACDecoder::decode_frame(size_t buffer_length, int16_t *output_buffer, uint32_t *num_samples) {
  ...
  *num_samples = 0;
  ...
  // Should never happen though, as we checked the the header crc
